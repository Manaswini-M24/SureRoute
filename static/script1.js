...
// Driver UI elements
const tripSelectionSection = document.getElementById('driver-trip-selection');
const liveTripSection = document.getElementById('driver-live-trip');
const tripListUl = document.getElementById('driver-trip-list');
const busIdSpan = document.getElementById('driver-bus-id');
const busNameSpan = document.getElementById('driver-bus-name');
const routeIdSpan = document.getElementById('driver-route-id');
const stopsListUl = document.getElementById('driver-stops-list');
...
// Dummy trips assigned to driver (replace with real backend integration)
const driverTrips = [
  {
    tripId: 'Route 101 - 8:00 AM',
    busId: 'Bus123',
...
function selectDriverTrip(trip) {
  currentDriverTrip = trip;
  currentStopIndex = 0;
  if (tripSelectionSection) tripSelectionSection.classList.add('hidden');
  if (liveTripSection) liveTripSection.classList.remove('hidden');
  updateLiveDriverTripView();
  showToast('Trip Selected', `Starting trip: ${trip.tripId}`, 'success');
}

function updateLiveDriverTripView() {
  if (busIdSpan) busIdSpan.textContent = currentDriverTrip.busId;
  if (busNameSpan) busNameSpan.textContent = currentDriverTrip.busName;
  if (routeIdSpan) routeIdSpan.textContent = currentDriverTrip.routeId;

  if (stopsListUl) {
    stopsListUl.innerHTML = '';
    currentDriverTrip.stops.forEach((stop, index) => {
      const li = document.createElement('li');
      li.textContent = stop;
...
// Mark stop departed
if (departedBtn) {
  departedBtn.addEventListener('click', () => {
    if (currentDriverTrip && currentStopIndex < currentDriverTrip.stops.length) {
      const currentStop = currentDriverTrip.stops[currentStopIndex];
      showToast('Stop Departed', `Departed from ${currentStop}`, 'success');
      
      currentStopIndex++;
      if (currentStopIndex === currentDriverTrip.stops.length) {
        showToast('Trip Completed', 'All stops completed successfully!', 'success');
        if (liveTripSection) liveTripSection.classList.add('hidden');
        if (tripSelectionSection) tripSelectionSection.classList.remove('hidden');
...
// Flag delay with reason
if (delayFlagBtn) {
  delayFlagBtn.addEventListener('click', () => {
    const reason = delayReasonSelect ? delayReasonSelect.value : '';
    if (!reason) {
      if (delayMessage) delayMessage.textContent = 'Please select a reason for the delay.';
      showToast('Selection Required', 'Please select a delay reason.', 'warning');
      return;
    }
    if (delayMessage) delayMessage.textContent = `Delay reported: ${reason}`;
    showToast('Delay Reported', `Delay reason: ${reason}`, 'warning');
    // Send delay info to backend here
  });
}

function showDriverTripSelection() {
  if (liveTripSection) liveTripSection.classList.add('hidden');
  if (tripSelectionSection) tripSelectionSection.classList.remove('hidden');
  currentDriverTrip = null;
  currentStopIndex = 0;
...
// Populate stops for Find My Bus based on route
function populateFindStops() {
